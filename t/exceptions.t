...
subtest 'Story: admin adds a closure for a specific date' => sub {
    ...
    my $row = $dbh->selectrow_hashref( q{SELECT * FROM staff_roster_exceptions WHERE roster_id = ? AND exception_date = ?},
...
subtest 'Story: admin edits an existing exception' => sub {
    my ($id) = $dbh->selectrow_array( q{SELECT id FROM staff_roster_exceptions WHERE roster_id = ? LIMIT 1}, undef, $rid );
    my @messages;
    my $cgi = StubCGI->new(
        {   roster_id      => $rid,
            exception_id   => $id,
    ...
    my $row
        = $dbh->selectrow_hashref( q{SELECT exception_type, reason FROM staff_roster_exceptions WHERE id = ?}, undef, $id );
...
subtest 'Story: delete only affects the targeted row in the roster' => sub {
    ...
    my $rows = $dbh->selectall_arrayref( q{SELECT id FROM staff_roster_exceptions WHERE roster_id = ? ORDER BY id},
        { Slice => {} }, $rid );
    is( scalar @{$rows}, 2, 'two exceptions exist before delete' );

    my $delete_id = $rows->[0]{id};
    @messages = ();
    $del_handler->( $plugin, $dbh, StubCGI->new( { roster_id => $rid, exception_id => $delete_id } ), \@messages );

    my ($remaining)
        = $dbh->selectrow_array( q{SELECT COUNT(*) FROM staff_roster_exceptions WHERE roster_id = ?}, undef, $rid );
...
subtest 'Story: cross-roster delete is a no-op (scoped to roster_id)' => sub {
    my ($other_id) = $dbh->selectrow_array( q{SELECT id FROM staff_roster WHERE id != ? LIMIT 1}, undef, $rid );
SKIP: {
        skip 'only one roster in DB', 1 if !$other_id;

        my ($id)
            = $dbh->selectrow_array( q{SELECT id FROM staff_roster_exceptions WHERE roster_id = ? LIMIT 1}, undef, $rid );
        my @messages;
        $del_handler->( $plugin, $dbh, StubCGI->new( { roster_id => $other_id, exception_id => $id } ), \@messages );
        my ($still_there)
            = $dbh->selectrow_array( q{SELECT COUNT(*) FROM staff_roster_exceptions WHERE id = ?}, undef, $id );
