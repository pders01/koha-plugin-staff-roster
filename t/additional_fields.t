...
$dbh->do( q{INSERT INTO additional_fields (id, tablename, name, repeatable) VALUES (?, ?, ?, ?)},
...
subtest 'Story: admin opens a brand-new roster form before any values exist' => sub {
    ...
    $dbh->do( q{DELETE FROM additional_field_values WHERE record_table = ? AND record_id = ?}, undef, 'staff_roster', $rid );

    my $af = $load->( $dbh, 'staff_roster', $rid );
    ok( $af->{available} && @{ $af->{available} }, 'available list contains the seeded field defs' );
...
subtest 'Story: admin saves a repeatable field with multiple values' => sub {
    ...
    my $af = $load->( $dbh, 'staff_roster', $rid );
    is_deeply( [ sort @{ $af->{values}{$ROSTER_F2} } ], [ 'q1', 'q2', 'urgent' ], 'all repeatable values stored' );
    is_deeply( $af->{values}{$ROSTER_F1}, ['BUD-42'], 'sibling single-value field still intact after the same save' );
};
...
subtest 'Story: clearing a value removes the row, save is a full replace' => sub {
    ...
    my $af = $load->( $dbh, 'staff_roster', $rid );
    is( scalar @{ $af->{values}{$ROSTER_F1} || [] }, 0, 'empty submission wipes the field' );
    is( scalar @{ $af->{values}{$ROSTER_F2} || [] },
        0, 'fields not in the submission are also wiped (full replace, not merge)' );
};
...
subtest 'Story: list view bulk-loads field values across multiple rosters' => sub {
    ...
    is_deeply( [ sort @{ $by_record->{$rid}{$ROSTER_F2} } ], [ 'a', 'b' ], 'bulk: repeatable values present' );
};
...
SKIP: {
        ...
        $save_map->( $dbh, 'staff_roster_assignments', $aid, { $ASSIGNMENT_F1 => [ 'CIRC', 'REF' ] } );

        my $af = $load->( $dbh, 'staff_roster_assignments', $aid );
        is_deeply( [ sort @{ $af->{values}{$ASSIGNMENT_F1} } ], [ 'CIRC', 'REF' ], 'JSON map persisted both values' );
        ...
        is( scalar @{ $af->{values}{$ASSIGNMENT_F1} || [] }, 0, 'empty array clears the field' );
...
subtest 'Story: deleting a record clears its additional field values' => sub {
    $save_cgi->( $dbh, 'staff_roster', $rid, StubCGI->new( { 'additional_field_' . $ROSTER_F1 => 'BUD-99' } ) );
...
subtest 'Story: tables with no field defs are a no-op' => sub {
    ...
    $save_cgi->( $dbh, $unused_table, 1, StubCGI->new( { 'additional_field_' . $ROSTER_F1 => 'should-not-stick' } ) );
    my ($leak)
        = $dbh->selectrow_array( q{SELECT COUNT(*) FROM additional_field_values WHERE record_table = ?},
        undef, $unused_table );
