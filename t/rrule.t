...
subtest '_rrule_from_params builds canonical RRULE strings' => sub {
    is( $build->( freq => 'WEEKLY', dows => [ 1, 3, 5 ] ),
        'FREQ=WEEKLY;BYDAY=MO,WE,FR', 'plain weekly multi-day' );

    is( $build->( freq => 'MONTHLY', dows => [1], ordinal => 1 ),
        'FREQ=MONTHLY;BYDAY=1MO', 'monthly first Monday' );

    is( $build->( freq => 'MONTHLY', dows => [ 1, 3 ], ordinal => -1 ),
        'FREQ=MONTHLY;BYDAY=-1MO,-1WE', 'monthly last Mon + last Wed' );

    is( $build->( freq => 'WEEKLY', dows => [1], interval => 2 ),
        'FREQ=WEEKLY;INTERVAL=2;BYDAY=MO', 'INTERVAL only emitted when > 1' );

    is( $build->( freq => 'WEEKLY', dows => [1], interval => 1 ),
        'FREQ=WEEKLY;BYDAY=MO', 'INTERVAL=1 stays implicit' );

    is( $build->( freq => 'WEEKLY', dows => [3], until_date => '2026-05-15' ),
        'FREQ=WEEKLY;BYDAY=WE;UNTIL=20260515T235959Z', 'UNTIL encoded as end-of-day UTC' );
...
subtest 'shim helpers stay backwards-compatible' => sub {
    ...
    is_deeply( $dows_from->('FREQ=MONTHLY;BYDAY=1MO,2WE'), [ 1, 3 ],
        'monthly: ordinals stripped, weekday ints recovered' );
    is_deeply( $byday_from->('FREQ=MONTHLY;BYDAY=1MO,-1FR'), [ 'MO', 'FR' ],
...
subtest '_rrule_label renders human-readable summaries' => sub {
    ...
    is( $label->('FREQ=WEEKLY;BYDAY=WE;UNTIL=20260515T235959Z'),
        'Wed (until 2026-05-15)', 'until suffix' );
    ...
    is( $label->('FREQ=MONTHLY;INTERVAL=2;BYDAY=3WE'),
        'Every 2 months: 3rd Wed of month', 'monthly with interval' );
